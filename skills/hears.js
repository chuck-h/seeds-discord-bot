...
module.exports = function(controller) {
  ...
  controller.hears(
    ...
    (bot, message) => {
      ...
      bot.reply(
        ...
        * To acknowledge gratitude tokens to someone send me a DM: \`\!acknowledge <seeds-account> [memo]\`.
  ...
  controller.hears("!acknowledge", ["direct_message"], (bot, message) => {
    let response;
    let sender = message.user;
    //let recipient = message.mentions.users.filter(user => user.bot === false).last();
    let pieces = message.text.split(" ");
    ...
    if (pieces.length == 3) {
      ...
      recipient = pieces[pieces.length - 1];
      memo = "";
    } else {
      bot.reply(message, "Usage: acknowledge <to> [memo]");
      return message.intent === "help";
    }
