...
module.exports.run = async (bot, message, args) => {
  if(args[1] == "help"){
    ...
    .setColor("#00ff00")
    .addField("help", "Usage: mysentacks")
  ...
  message.channel.send(`Alright, I will PM you the gratitude acks you sent in the last ${size} messages.`)  
  var count=0;
  for (var i = 0; i < res.actions.length; i++) {
    ...
    if (from === account) {
      message.author.send(`On ${timestamp}, to: ${to}: "${memo}"`);
      count++;
    }
    
  }
  message.author.send(`Total ${count} acks sent.`);
};
